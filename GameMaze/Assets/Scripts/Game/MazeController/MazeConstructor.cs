            ...
            for (int j = 0; j < maze.GetLength(1); j++)
            {
                ...
                else if (maze[i, j] == 1)
                {
                    try//исключени€, что бы не вызывать ошибку когда пытаетс€ получить значаени€ элемента < 0 или больше максимального размера лаберинта
                    {
                        ...
                        {
                            tilemapWall.SetTile(new Vector3Int(i, j, 0), wallLeftDownTile);//—озаднаие боковой стены слева в низ
                        }
                        else if (maze[i - 1, j] == 0 && maze[i + 1, j] == 1 && maze[i, j + 1] == 0 && maze[i, j - 1] == 1)
                        {
                            tilemapWall.SetTile(new Vector3Int(i, j, 0), wallRightDownTile);//—озаднаие боковой стены справа в низ
                        }
                        else if (maze[i - 1, j] == 1 && maze[i + 1, j] == 0 && maze[i, j + 1] == 1 && maze[i, j - 1] == 0)
                        {
                            tilemapWall.SetTile(new Vector3Int(i, j, 0), wallLeftUpTile); //—озаднаие боковой стены слева в верх
                        }
                        else if (maze[i - 1, j] == 0 && maze[i + 1, j] == 1 && maze[i, j + 1] == 1 && maze[i, j - 1] == 0)
                        {
                            tilemapWall.SetTile(new Vector3Int(i, j, 0), wallRightUpTile); //—озаднаие боковой стены справа в верх
                        }
